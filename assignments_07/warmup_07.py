...
tools = [
    {
        ...
        'function': {
            ...
            'description': 'Converts the temperature from celsius to fahrenheit and returns it.',
            'parameters': {
                'type': 'object',
                'properties': {
                    "celsius": {
                        "type": "number",
                        "description": "temperature input in celsius",
                    }
                },
                'required': ["celsius"],
            },
...
def run_agent(user_prompt: str) -> str:
    ...
    SYSTEM_PROMPT = '''You are a simple assistant that can tell the current time and convert the temperature from celsius to fahrenheit.
                     Use the tool get_current_time whenever a user asks about the time.  Use the tool celsius_to_fahrenheit whenever a user asks to convert celsius to fahrenheit '''
    ...
    if first_message.tool_calls:
        ...
        for tool_call in first_message.tool_calls:
            ...
            if function_name == 'get_current_time':
                ...
                celsius_parameter = json.loads(tool_call.function.arguments)['celsius']
                tool_result = celsius_to_fahrenheit(celsius_parameter)
            else:
...
#Q3
#Updated tools list and run_agent
response_a = run_agent("What is 37 degrees Celsius in Fahrenheit?")
print("Response A:", response_a)
#Q: Explaining whether a tool was called and why.
#A: A tool was called because the system prompt explicitly instructs to use the tool celsius_to_fahrenheit whenever the user asks to convert the celsius to fahrenheit. 

response_b = run_agent("What is the boiling point of water in plain English?")
print("Response B:", response_b)
#Q: Explaining whether a tool was called and why.
#A: Tool won't be called because it's not asking the agent to convert a temperature
...
class CsvManager:
    ...
    def compute_correlation(self, col1: str, col2: str):
        ...
        if col1 not in self.df.columns:
            return {"error": f"Error: column '{col1}' is not in {self.df.columns.tolist()}"}

        if col2 not in self.df.columns:
            return {"error": f"Error: column '{col2}' is not in {self.df.columns.tolist()}"}
...
tools_schema = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Computes Pearson correlation between two columns in the loaded DataFrame (uses scipy.stats.pearsonr).",
            "parameters": {
                "type": "object",
                "properties": {
                    "col1": {
...
#Q: Add a comment above the print that identifies what each role (system, user, assistant, tool) represents in the ReAct loop.
#A: The system sets the assistant's personality, boundaries, and when to use the tools.  The user sends a query to the assistant to solve.  The assistant makes decision on whether to use any tools or respond directly.  The tool provides results from the Python function to the assistant.     
...
def compute_correlation(col1: str, col2: str) -> dict:
    ...
    Args:
        col1: The name of the column 1 to calculate correlation.
        col2: The name of the column 2 to calculate correlation.

    Returns:
        A dictionary containing col1, col2, the correlation coefficient and p-value or an error dict.
    """
...
#Q: Add a comment comparing what smolagents generates automatically to the JSON schema you wrote manually in Q4. What information does smolagents need from you (the developer) in order to produce a good description?
#A: Smolagents automatically creates the JSON schema for the tools but it requires a detailed description of the function as well as the parameter types and return types.
...
#Q: What did each agent actually produce? Did the ToolCallingAgent change the dot color? Did the CodeAgent?
#ToolCallingAgent just used the pre-made plotting tool which did not have a setting for color but it hallucinated that it did plot with the green dots.  CodeAgent did use matplotlib to create the custom python code to change the dots to green.

#Q: What does this reveal about when each type of agent is more useful?
#A: Tool-Calling Agents are useful for simple and safe tasks.  Code Agents are useful for custom styling or complex analysis.


#Q9

#Q: Describe a task where a ToolCallingAgent would be a better choice than a CodeAgent. What property of the task makes it a good fit for a tool-based approach?
#A: Booking a flight.  What makes it a task a good fit for a tool-based approach is having the strict rules with no ambiguities.
...
#A: Prompt injection because since the CodeAgent can dynamically write and runs Python scripts, it can be tricked into performing malicious activities like deleting the database or exposing the sensitive information.
